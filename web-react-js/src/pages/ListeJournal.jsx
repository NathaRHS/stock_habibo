...
function ListeJournal() {
  ...
  useEffect(() => {
    ...
    async function charger() {
      if (!token) {
        setError("Connectez-vous pour consulter les sessions.");
        setLoading(false);
        return;
      }
      try {
        ...
        if (!response.ok)
          throw new Error(
            `Impossible de récupérer les sessions (${response.status}).`,
          );
  ...
  const journauxFiltres = useMemo(() => {
    ...
    return journaux.filter((journal) => {
      const statutOk =
        filtre === "TOUS" || normaliser(journal.statut) === filtre;
      const contenu = normaliser(
        [
          journal.reference,
          journal.nomClient,
          journal.fournisseur,
          journal.typeMouvementJournal,
          journal.statut,
        ].join(" "),
      );
    ...
    <div className="journal-page-shell">
      ...
      <section className="journal-page-workspace">
        ...
        <main className="journal-list-page">
          ...
          {error && (
            <div className="journal-list-error" role="alert">
              {error}
            </div>
          )}
          ...
          <section className="journal-list-card">
            <div className="journal-list-toolbar">
              <nav
                className="journal-filter-tabs"
                aria-label="Filtrer par statut"
              >
                {FILTRES.map((statut) => (
                  <button
                    className={filtre === statut ? "is-active" : ""}
                    key={statut}
                ...
                <table className="journal-table">
                  <thead>
                    <tr>
                      ...
                      <th>Document</th>
                      <th>Statut</th>
                      <th className="journal-action-cell">Action</th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <footer className="journal-list-footer">
              <span>
                {journauxFiltres.length} affiché
                {journauxFiltres.length > 1 ? "s" : ""}
              </span>
              <span>{journaux.length} au total</span>
            </footer>
          </section>
        </main>
      </section>
...
function JournalRow({ journal, filtre }) {
  const statut = normaliser(journal.statut);
  const tonalite = statut.includes("VALIDE")
    ? "success"
    : statut.includes("MODIFIE")
      ? "danger"
      : statut.includes("ATTENTE")
        ? "warning"
        : "progress";
  const documentNom = String(journal.urlPieceJointe ?? "")
    .replaceAll("\\", "/")
    .split("/")
    .filter(Boolean)
    .at(-1);
  const partenaire =
    journal.fournisseur || journal.nomClient || "Non renseigné";
  const sortie = normaliser(journal.typeMouvementJournal).includes("SORTIE");

  return (
    <tr>
      <td>
        <div className="journal-reference-cell">
          <div>
      ...
      <td>
        <div className="journal-partner-cell">
          ...
          <div>
            ...
            <small>{journal.fournisseur ? "Fournisseur" : "Client"}</small>
          </div>
        </div>
      </td>
      <td>
        <span className="journal-flow">
        
          {journal.typeMouvementJournal || "Non renseigné"}
        </span>
      </td>
      <td>
        <span
      ...
      <td>
        <span className={`journal-status journal-status-${tonalite}`}>
          <i />
          {journal.statut}
        </span>
      </td>
      <td className="journal-action-cell">
        {journal.typeMouvementJournal === "SORTIE" ? (
          <Link
            className="journal-open-link"
